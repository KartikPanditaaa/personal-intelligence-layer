---
Status: Draft
Owner: Founder
Last Updated: 2026-08-02
Related Documents:
  - "[Privacy](../10-privacy/README.md)"
  - "[Security](../09-security/README.md)"
  - "[Risk Register](../00-company/25-risk-register.md)"
  - "[Data Model](../04-architecture/12-data-model.md)"
Tags: business, legal, compliance
Source: Migrated verbatim from `blueprint/05-business-ops/23-legal-and-compliance-notes.md`
---

# 23 Legal and Compliance Notes

## Decided

Privacy is a core product principle.

Enterprise trust was discussed extensively.

## Reasoned

Likely future focus areas: - Data handling - User consent - Local
processing - Memory controls

## Requires founder decision

Specific compliance targets (GDPR, SOC 2, HIPAA, etc.).
