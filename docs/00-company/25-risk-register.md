---
Status: Draft
Owner: Founder
Last Updated: 2026-08-02
Related Documents:
  - [Scalability and Risk](../04-architecture/14-scalability-and-risk.md)
  - [Assumptions to Validate](../02-research/26-assumptions-to-validate.md)
  - [Red Team Checklist](../18-templates/RED_TEAM_CHECKLIST.md)
  - [Privacy](../10-privacy/README.md)
  - [Security](../09-security/README.md)
Tags: company, risk, execution
Source: Migrated verbatim from `blueprint/06-risk-execution/25-risk-register.md`
---

# 25 Risk Register

## Risks Discussed

### Decided

-   Privacy concerns
-   Enterprise trust
-   Context loss
-   AI dependency
-   Feature creep
-   Building too broadly too early

### Reasoned

Severity and likelihood require formal scoring.

## Mitigations Discussed

-   Research-first approach
-   Product constitution
-   Trust-first design
-   User-controlled memory
-   Red-team reviews

## Requires founder decision

Risk ownership and prioritization.
